...
class GroupingReturService {
  ...
  static Future<List<GroupingRetur>> fetchRecords({
    ...
    try {
      ...
      final uri = Uri.parse('$baseUrl/grouping-returs')
      ...
      if (response.statusCode == 200) {
        ...
        return recordsJson
            .map((json) => GroupingRetur.fromJson(json))
            .toList();
      } else {
  ...
  static Future<GroupingReturStatistics> fetchStatistics({
    String? date,
  }) async {
    try {
      final token = AuthService.token;
      ...
      final uri = Uri.parse('$baseUrl/grouping-returs/statistics')
          .replace(queryParameters: queryParams);
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return GroupingReturStatistics.fromJson(data['data']);
      } else {
  ...
  static Future<void> createRecord(Map<String, dynamic> data) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/grouping-returs'),
        headers: {
          'Authorization': 'Bearer $token',
  ...
  static Future<void> updateRecord(int id, Map<String, dynamic> data) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('$baseUrl/grouping-returs/$id'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'Accept': 'application/json',
