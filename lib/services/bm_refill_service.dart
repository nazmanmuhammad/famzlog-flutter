...
class BmRefillService {
  ...
  static Future<List<BmRefill>> fetchRecords({
    ...
    try {
      ...
      final uri = Uri.parse('$baseUrl/bm-refills')
      ...
      if (response.statusCode == 200) {
        ...
        return recordsJson
            .map((json) => BmRefill.fromJson(json))
            .toList();
      } else {
  ...
  static Future<BmRefillStatistics> fetchStatistics({
    String? date,
  }) async {
    try {
      final token = AuthService.token;
      ...
      final uri = Uri.parse('$baseUrl/bm-refills/statistics')
          .replace(queryParameters: queryParams);
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return BmRefillStatistics.fromJson(data['data']);
      } else {
  ...
  static Future<void> updateRecord(int id, Map<String, dynamic> data) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('$baseUrl/bm-refills/$id'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'Accept': 'application/json',
