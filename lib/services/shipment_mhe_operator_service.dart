...
class ShipmentMheOperatorService {
  static const String baseUrl = 'http://10.97.120.57:9000/api';

  static Future<List<ShipmentMheOperator>> fetchRecords({
    String? search,
    String? date,
  }) async {
    try {
      ...
      final uri = Uri.parse('$baseUrl/shipment-mhe-operators')
      ...
      if (response.statusCode == 200) {
        ...
        return recordsJson
            .map((json) => ShipmentMheOperator.fromJson(json))
            .toList();
      } else {
  ...
  static Future<ShipmentMheOperatorStatistics> fetchStatistics({
    String? date,
  }) async {
    try {
      final token = AuthService.token;
      ...
      final uri = Uri.parse('$baseUrl/shipment-mhe-operators/statistics')
          .replace(queryParameters: queryParams);
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return ShipmentMheOperatorStatistics.fromJson(data['data']);
      } else {
  ...
  static Future<void> createRecord(Map<String, dynamic> data) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/shipment-mhe-operators'),
        headers: {
          'Authorization': 'Bearer $token',
  ...
  static Future<void> updateRecord(int id, Map<String, dynamic> data) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('$baseUrl/shipment-mhe-operators/$id'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'Accept': 'application/json',
