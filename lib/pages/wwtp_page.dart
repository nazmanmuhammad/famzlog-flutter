...
import 'package:famzlog_flutter/pages/wwtp_form_page.dart';
import 'package:famzlog_flutter/widgets/modern_snackbar.dart';
import 'package:intl/intl.dart';

class WwtpPage extends StatefulWidget {
  const WwtpPage({super.key});

  @override
  State<WwtpPage> createState() => _WwtpPageState();
}

class _WwtpPageState extends State<WwtpPage> {
  List<Wwtp> _records = [];
  WwtpStatistics? _statistics;
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final records = await WwtpService.getWwtps(
        warehouseId: warehouseId,
        date: _selectedDate,
        pickerName: _searchController.text.isNotEmpty ? _searchController.text : null,
      );
      final stats = await WwtpService.getStatistics(
        warehouseId: warehouseId,
        date: _selectedDate,
      );
  ...
  Future<void> _deleteRecord(int id) async {
    ...
    if (confirm == true) {
      try {
        await WwtpService.deleteWwtp(id);
        if (mounted) {
          showModernSnackBar(
            context,
            title: 'Berhasil',
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final result = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const WwtpFormPage(),
            ),
          );
          if (result == true) {
            _loadData();
  ...
  Widget _buildRecordCard(Wwtp record) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    final result = await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => WwtpFormPage(wwtp: record),
                      ),
                    );
                    if (result == true) {
                      _loadData();
            ...
            _buildInfoRow('Material', '${record.material?.nama ?? '-'} (${record.material?.uom ?? '-'})', Icons.category),
            _buildInfoRow('Picker', record.pickerName, Icons.person),
