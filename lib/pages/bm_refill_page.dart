...
import 'package:famzlog_flutter/pages/bm_refill_form_page.dart';
import 'package:famzlog_flutter/widgets/modern_snackbar.dart';
import 'package:intl/intl.dart';

class BmRefillPage extends StatefulWidget {
  const BmRefillPage({super.key});

  @override
  State<BmRefillPage> createState() => _BmRefillPageState();
}

class _BmRefillPageState extends State<BmRefillPage> {
  List<BmRefill> _records = [];
  BmRefillStatistics? _statistics;
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    ...
    try {
      final records = await BmRefillService.fetchRecords(
        search: _searchController.text.isNotEmpty ? _searchController.text : null,
        date: _selectedDate,
      );
      final stats = await BmRefillService.fetchStatistics(
        date: _selectedDate,
      );
  ...
  Future<void> _deleteRecord(int id) async {
    ...
    if (confirm == true) {
      try {
        await BmRefillService.deleteRecord(id);
        if (mounted) {
          showModernSnackBar(
            context,
            title: 'Berhasil',
                ...
                children: [
                  Expanded(
                    child: _buildStatCard(
                      'Total Bin',
                      NumberFormat('#,###', 'id_ID').format(_statistics!.totalBins),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final result = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const BmRefillFormPage(),
            ),
          );
          if (result == true) {
            _loadData();
  ...
  Widget _buildRecordCard(BmRefill record) {
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    final result = await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => BmRefillFormPage(record: record),
                      ),
                    );
                    if (result == true) {
                      _loadData();
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${NumberFormat('#,###', 'id_ID').format(record.binCount)} Pcs',
                        style: const TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          color: Color(0xFF1C84C2),
