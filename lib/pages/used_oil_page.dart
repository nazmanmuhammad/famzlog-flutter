...
class UsedOilPage extends StatefulWidget {
  const UsedOilPage({super.key});

  @override
  State<UsedOilPage> createState() => _UsedOilPageState();
}

class _UsedOilPageState extends State<UsedOilPage> {
  List<UsedOil> _records = [];
  UsedOilStatistics? _statistics;
  bool _loading = true;
  ...
  Future<void> _deleteRecord(int id) async {
    ...
    if (confirm == true) {
      try {
        await UsedOilService.deleteUsedOil(id);
        if (mounted) {
          showModernSnackBar(
            context,
            title: 'Berhasil',
                ...
                children: [
                  Expanded(
                    child: _buildStatCard(
                      'Oil IN',
                      _statistics!.totalUsedOilIn.toStringAsFixed(2),
                      Colors.green,
                  ...
                  Expanded(
                    child: _buildStatCard(
                      'Oil OUT',
                      _statistics!.totalUsedOilOut.toStringAsFixed(2),
                      Colors.red,
                      Icons.arrow_upward,
                    ),
                  ),
                  ...
                  Expanded(
                    child: _buildStatCard(
                      'Balance',
                      _statistics!.balance.toStringAsFixed(2),
                      Colors.blue,
                      Icons.balance,
                    ),
                  ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final result = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const UsedOilFormPage(),
            ),
          );
          if (result == true) {
            _loadData();
  ...
  Widget _buildRecordCard(UsedOil record) {
    return Card(
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    final result = await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => UsedOilFormPage(usedOil: record),
                      ),
                    );
                    if (result == true) {
                      _loadData();
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        record.usedOilIn.toStringAsFixed(2),
                        style: const TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          color: Colors.green,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        record.usedOilOut.toStringAsFixed(2),
                        style: const TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          color: Colors.red,
